...
for f in ./env/*; do 
  ...
  # Run the cdxgen docker image with the environment variables from ./env/$filename.env
  container_id="$(docker run  --env-file ./env/$filename.env  -d -v maven-deps:/root/.m2 cdxgen)"
