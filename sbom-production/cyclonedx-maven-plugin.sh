...
for f in ./study-subjects-env/*; do 
  ...
  # Run the cyclonedx-maven-plugin docker image with the environment variables from ./study-subjects-env/$filename.env
  container_id="$(docker run  --env-file ./study-subjects-env/$filename.env  -d -v maven-deps:/root/.m2 cyclonedx-maven-plugin)"
